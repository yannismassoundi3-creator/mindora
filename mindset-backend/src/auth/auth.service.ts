...
export class AuthService {
  ...
  async login(dto: LoginDto) {
    ...
    // Vérifier si Brevo est configuré. Sinon, bypass 2FA pour ne pas bloquer l'utilisateur.
    const brevoApiKey = this.configService.get<string>('BREVO_API_KEY');
    if (!brevoApiKey) {
      console.log(`[AUTH] 2FA bypassé pour ${user.email} car BREVO_API_KEY n'est pas configuré.`);
